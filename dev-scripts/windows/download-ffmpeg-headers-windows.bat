...
set OUTPUT_DIR=%PROJECT_ROOT%\external\ffmpeg\windows
...
echo Target:      Windows compilation
...
echo.
echo You can now build for Windows without installing
echo FFmpeg development packages.
echo.
echo Next step (MSYS2/MinGW):
echo   cmake -B build\windows -G "MinGW Makefiles" -DCMAKE_BUILD_TYPE=Release
echo   make -C build\windows
echo.
echo Next step (Visual Studio):
